...
export default function Contact() {
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif', paddingTop: 72 }}>
      ...
      <section style={{ padding: '80px 24px', background: '#F5F7FA' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'start' }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: '40px', border: '1px solid #E2E8F0' }}>
            ...
            {/* Note: For production, connect this to a backend API route or Formspree */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[{ label: 'Full Name', type: 'text', placeholder: 'Your full name' }, { label: 'Email Address', type: 'email', placeholder: 'your@email.com' }, { label: 'Phone Number', type: 'tel', placeholder: '0xx xxx xxxx' }].map(f => (
                <div key={f.label}>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#475569', marginBottom: 6 }}>{f.label}</label>
                  <input type={f.type} placeholder={f.placeholder} style={{ width: '100%', padding: '12px 16px', borderRadius: 8, border: '1px solid #E2E8F0', fontSize: 15, color: '#0A1F44', outline: 'none', fontFamily: 'DM Sans, sans-serif', transition: 'border-color 0.2s', boxSizing: 'border-box' }} />
                </div>
              ))}
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#475569', marginBottom: 6 }}>Message / Reason for Contact</label>
                <textarea rows={4} placeholder="I would like to book an appointment / enquire about..." style={{ width: '100%', padding: '12px 16px', borderRadius: 8, border: '1px solid #E2E8F0', fontSize: 15, color: '#0A1F44', outline: 'none', fontFamily: 'DM Sans, sans-serif', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              <button type="button" style={{ background: '#0A1F44', color: 'white', padding: '14px 28px', borderRadius: 8, border: 'none', fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'DM Sans, sans-serif' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#12326B'; }}
      ...
      {/* Map */}
      <div style={{ height: 400, background: '#E8F4FD', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
